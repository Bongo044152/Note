...
前面有一張圖片，呈現資料庫的table以及row和column，這些概念和電子表格非常相似，但資料庫和電子表格之間還有一些重要的區別。
...
非結構化資料庫（NoSQL）是一種資料庫系統，它不使用結構化查詢語言（SQL）來存取資料，而是使用非結構化數據格式，如JSON、XML、HTML等。這種資料庫系統適用於儲存和處理非結構化資料，如網頁、圖片、音頻、視頻等。
